...
export const sseClients: Record<Group, Set<Response>> = {
  bus: new Set(),
  train: new Set(),
};
...
async function fetchFeed(key: string, url: string): Promise<VehiclePosition[]> {
  ...
  for (const entity of obj.entity || []) {
    ...
    const veh = v.vehicle || {};
    // REF-5: use || so a vehicle is skipped when *either* coordinate is
    // missing/zero, not only when both are.
    if (!pos.latitude || !pos.longitude) continue;
    // Discard positions outside Malaysia's approximate bounding box.
    // Corrupt protobuf decodes occasionally produce (0,0) or wildly off-range
    // coordinates that would pollute the ETA calculations.
    if (
      pos.latitude < 0.85 ||
      pos.latitude > 7.4 ||
      pos.longitude < 99.6 ||
    ...
    positions.push({
      ...
      currentStopSequence:
        v.currentStopSequence ?? v.current_stop_sequence ?? undefined,
...
async function pollGroup(group: Group): Promise<void> {
  ...
  const results = await Promise.allSettled(
    feeds.map((f) => fetchFeed(f.key, f.url)),
  );

  const positions = results.flatMap((r, i) => {
    if (r.status === "fulfilled") return r.value;
    console.warn(
      `[poll] ${feeds[i].key} failed: ${(r.reason as Error)?.message}`,
    );
    return [];
  });
  ...
  console.log(
    `[poll] ${group}: ${positions.length} vehicles — ${new Date().toISOString()}`,
  );
  ...
  for (const client of sseClients[group]) {
    try {
      ...
      sseClients[group].delete(client);
    }
  }
}
